...
export default class StorageManager {
	...
	async saveMenuImage(mid, imageVersion, imageBase64) {
		try {
			if (!this.db) throw new Error("Database non aperto.");
			const query =
				"INSERT OR REPLACE INTO MenuImage (mid, imageVersion, imageBase64) VALUES (?, ?, ?);";
			await this.db.runAsync(query, [mid, imageVersion, imageBase64]);
			console.log(`Immagine del menu con mid ${mid} salvata con successo.`);
		} catch (error) {
			console.error(
				`Errore durante il salvataggio dell'immagine del menu con mid ${mid}:`,
				error
			);
		}
	}
	...
	async isImageUpToDate(mid, imageVersion) {
		try {
			...
			const isUpToDate = result
				? String(result.imageVersion) === String(imageVersion)
				: false;
			...
			console.error(
				`Errore durante la verifica dell'immagine con mid ${mid}:`,
				error
			);
	...
	async deleteMenuImage(mid) {
		try {
			...
			console.error(
				`Errore durante l'eliminazione dell'immagine del menu con mid ${mid}:`,
				error
			);
		}
	}
	...
	async getMenuImage(mid) {
		try {
			...
			console.error(
				`Errore durante il recupero dell'immagine del menu con mid ${mid}:`,
				error
			);
			return null;
		}
